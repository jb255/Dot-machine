...
#define write_buffer_close(line1, dot)               \
    dot.bits.color = Close;                          \
    write_board_close(line1, dot);                   \
    *(buffer_dot.content + buffer_dot.lenght) = dot; \
    if (buffer_dot.lenght < XY_MAX) {                \
        buffer_dot.lenght += 1;                      \
    }

/*! The function `init_board` initialize a new board. */
void init_board(void) {
    ...
    while (piece.bits.y < Y_MAX) {
        ...
        while (piece.bits.x < X_MAX) {
            Line *line1 = board.y_all + piece.bits.y;

            line1->x_bits.oob = 0b11111;
            write_buffer_close(line1, piece);
            piece.bits.x += 1;
        }
        piece.bits.y += 1;
    }
